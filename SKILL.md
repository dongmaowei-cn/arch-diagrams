...
边是**贴节点边界中点的直角折线**，不是随手拉的线。三条硬规则：

1. **起点/终点落在节点四边中点**，永远不从角部出发（"线从角上擦着落下来"是脏观感）
2. **一条边的入箭头终点 ≠ 另一条边的出边起点**——请求与回执挤在同一个边界点是碰撞，要错开或用不同边
3. **箭头尖端必须够到目标边界**——悬空停在半空（8px+）算没画完

三条**内部纪律**（端点之外，线/框/标签与节点的干涉）：

4. **曲线/自环必须凸出节点外侧**——弧线或贝塞尔不能"弯进节点身体"（端点贴边看不出来，弧线中段却探进矩形内部）。画自环时凸到节点外侧再弯回
5. **composite-bg 框内节点必须完整在框内**——子状态机/组合框是语义容器，节点探出框边（几像素都算）= 布局没对齐
6. **边标签不压节点**——edge-label / t-lbl 锚点不得落在节点内部。微服务/架构图的共享总线画在服务卡片之上时，骑线标签（所属边本身穿过该节点）豁免
7. **关系线不穿表体（ER）**——`.er-edge` 必须正交绕行、走表间走廊，不得斜穿/横穿任何表体
8. **时序图激活条必须盖住消息（时序）**——每条贴 lifeline 的消息端点（起/终）必须在某激活条 y 区间内（±6px），且端点贴条左/右边缘（±2px），不在条中心/lifeline 上。"激活条没盖住最后一条自环"和"消息从激活条中心进出"是同一类高频返工

例外：时序图消息箭头与 lifeline 保持 ~5px 设计间距；跨节点上方经过的**穿行线**允许 8-12px 间距，但**贴节点的连接边必须贴边**。改完跑 selftest 第 8 项（边几何校验，`shared/edge-check.py`）会把 1-8 一次拦住——这八条正是 `shared/coordinate-system.md` 关键约定第 4 条的落地。
...
**首选**：跑 `shared/selftest.sh`，8 项检查一次全过：
...
8. **边几何校验**（`shared/edge-check.py`，9 项子检查）：
   - 端点 4 项：起点/终点落在节点边界（中点，非角部）或边交汇点上、箭头够到目标、不存在"入箭头终点 = 出边起点"的连接点碰撞
   - 内部 5 项：曲线自环不弯进节点（[5]）、composite 框内节点完整在框内（[6]）、边标签不压节点（[7]，bus 线骑线标签豁免）、ER 关系线不穿表体（[8]，`.er-edge` 正交绕行）、时序激活条盖住消息且端点贴条边（[9]）
   - 无 python3 时跳过（标识为 `○`）

任一不通过就修复后再跑，直到全过才报告产物。

> 3/4/5 项作为防御性检查保留:防止用户从 gallery 页误粘 B 区图鉴内容进产物。
> 第 8 项拦的是"线从角部出发 / 请求回执共用连接点 / 箭头悬空 / 曲线弯进节点 / 子状态机出框 / 标签压节点 / 关系线穿表体 / 激活条没盖住消息"八类几何问题。
